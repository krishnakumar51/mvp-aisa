...
def run_agent3(seq_no: str, platform: str) -> dict:
    ...
    agent2_dir = Path("generated_code") / seq_no / "agent2"
    agent3_dir = Path("generated_code") / seq_no / "agent3"
    ...
    if platform == "mobile":
        ...
        if sys.platform == "win32":
            ...
            call "{activate_script}" & pip install --upgrade pip > nul & pip install -r "{reqs_path}" > nul
            echo.
            ...
            "{python_executable}" -m venv env
            echo "[2/4] Activating & installing dependencies..."
            source "{activate_script}" && pip install --upgrade pip > /dev/null && pip install -r "{reqs_path}" > /dev/null
            echo ""
            echo "Waiting 10s for Appium server to start..."
            sleep 10
            echo "[3/4] Running automation script..."
